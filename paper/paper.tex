...
\begin{algorithm}[ht!]
  ...
  \lIf{$max(t_a,t_b) < 1$ \textbf{or} $t_a^\prime < t_a$ \textbf{or} $t_b^\prime < t_b$} {
		\textbf{  break}
	}
...
\begin{figure*} %This just must appear at the page before the chapter "Displaying the average curve"
	\centering
		\begin{tabular}{cccc}
			...
			\small a) no geodesic sampling & \small b) with geodesic sampling 
			  & \small c) the closest projections & \small d) net of circular arcs
		\end{tabular}
